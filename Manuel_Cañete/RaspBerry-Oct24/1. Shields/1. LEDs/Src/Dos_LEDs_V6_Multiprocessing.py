...
'''
Explicació del codi:
    setup_gpio(pin): La configuració GPIO s'aplica dins de cada procés per evitar conflictes entre processos.
    blink_led1() i blink_led2(): Cada funció de parpelleig s'executa en un procés separat. Així, cada LED és controlat per un procés independent, executant-se realment en paral·lel.
    multiprocessing.Process: Creem un procés per a cada LED i l’iniciem amb start().
    join(): Mantenim el programa actiu perquè els processos continuïn executant-se.
Aquest enfocament amb multiprocessing fa que els dos LEDs funcionin de manera completament independent, aprofitant els nuclis de la CPU si hi són disponibles.
'''